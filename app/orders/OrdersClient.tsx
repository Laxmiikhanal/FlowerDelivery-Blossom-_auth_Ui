...
export default function OrdersClient() {
  ...
  const reorder = (order: Order) => {
    ...
    const existingCart: CartItem[] = existingRaw
      .map(normalizeItem)
      .filter((x: CartItem) => Boolean(x.id));

    const map = new Map<string, CartItem>();
    for (const it of existingCart) {
      map.set(it.id, it);
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-purple-50">
        <div className="mx-auto max-w-6xl px-6 py-10">
          <div className="h-10 w-56 animate-pulse rounded-xl bg-pink-100" />
          <div className="mt-6 h-44 animate-pulse rounded-3xl border border-pink-100 bg-white" />
          <div className="mt-4 h-44 animate-pulse rounded-3xl border border-pink-100 bg-white" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-white to-purple-50">
      <header className="sticky top-0 z-40 border-b border-pink-200 bg-white/85 shadow-sm backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <Link href="/home" className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="-mt-1 text-xs text-gray-500">My Orders</p>
            </div>
          </Link>

          <nav className="hidden items-center gap-7 text-sm font-medium text-gray-700 md:flex">
            <Link href="/home" className="transition hover:text-pink-600">
              Home
            </Link>
            <Link href="/products" className="transition hover:text-pink-600">
              Shop
            </Link>
            <Link href="/cart" className="transition hover:text-pink-600">
              Cart
            </Link>
            <Link href="/orders" className="font-semibold text-pink-600">
              Orders
            </Link>
          </nav>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link
            href="/home"
            className="inline-flex items-center justify-center rounded-2xl border border-pink-200 bg-gradient-to-r from-pink-500 to-pink-600 px-5 py-3 text-sm font-semibold text-white shadow hover:opacity-95 transition"
          >
            Continue shopping
          </Link>
        </div>
        ...
        {orders.length === 0 && (
          <div className="mt-10 rounded-3xl border border-pink-200 bg-white p-10 text-center shadow-sm">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-pink-100 bg-pink-50 text-2xl">
              📦
            </div>
            ...
            <Link
              href="/home"
              className="mt-5 inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-pink-500 to-pink-600 px-5 py-3 text-sm font-semibold text-white shadow hover:opacity-95 transition"
            >
              Start shopping
            </Link>
          </div>
        ...
        <div className="mt-8 space-y-6">
          {orders.map((order) => {
            ...
            return (
              ...
              >
                <div className="border-b border-pink-100 bg-gradient-to-r from-pink-50 to-purple-50 px-6 py-5">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <div className="rounded-xl border border-pink-100 bg-white px-3 py-2 text-sm">
                        <p className="text-xs text-gray-500">Items</p>
                        <p className="font-semibold text-gray-900">{orderItemCount}</p>
                      </div>
                      ...
                      {order.status === "Processing" ? (
                        ...
                        <button
                          disabled
                          className="cursor-not-allowed rounded-2xl bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-600"
                          type="button"
                        >
                          Cancel
                        </button>
                ...
                <div className="p-6">
                  {order.items.length === 0 ? (
                    <div className="rounded-2xl border border-pink-100 bg-pink-50/30 px-4 py-3 text-sm text-gray-600">
                      This order has no item details available.
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      {order.items.map((it) => (
                        ...
                        >
                          <div className="relative h-24 w-24 overflow-hidden rounded-2xl border border-pink-100 bg-gray-100">
                            <Image
